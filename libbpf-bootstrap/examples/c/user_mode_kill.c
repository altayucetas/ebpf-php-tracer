...
int main(int argc, char **argv) {
    ...
    struct user_mode_kill_bpf *skel;
    ...
    skel = user_mode_kill_bpf__open_and_load();
    if(!skel) {
    ...
    err = user_mode_kill_bpf__attach(skel);
    if (err) {
    ...
    urb = user_ring_buffer__new(bpf_map__fd(skel->maps.user_ringbuf), 
       NULL);
    ...
    if(mode->mode_id == PROFILE_MODE) {
        krb = ring_buffer__new(bpf_map__fd(skel->maps.ring_buff), 
        ...
        printf("Syscall-Function file is successfully opened for writing\n");
        ...
        printf("Syscall-Function file is successfully opened for reading\n");
...
cleanup:
    ...
    user_mode_kill_bpf__destroy(skel);
    return err < 0 ? -1 : 0;
}